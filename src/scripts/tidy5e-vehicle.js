...
export class Tidy5eVehicle extends dnd5e.applications.actor
  .ActorSheet5eVehicle {
  static get defaultOptions() {
    ...
    return mergeObject(super.defaultOptions, {
      ...
      width: game.settings.get("foundryvtt-tidy5e-sheet-nolat", "vehicleSheetWidth") ?? 740,
      height: 720,
      tabs: [
        {
          navSelector: ".tabs",
  ...
  async _renderInner(...args) {
    ...
    if (game.modules.get("character-actions-list-5e")?.active)
      injectVehicleSheet = game.settings.get(
...
async function setSheetClasses(app, html, data) {
  ...
  if (
    ...
    html
      .find(".tidy5e-sheet.tidy5e-vehicle .profile")
      .addClass("roundPortrait");
  }
  if (game.settings.get("foundryvtt-tidy5e-sheet-nolat", "hpOverlayBorderVehicle") > 0) {
    $(".system-dnd5e")
      .get(0)
      .style.setProperty(
        "--vehicle-border",
  ...
  if (game.settings.get("foundryvtt-tidy5e-sheet-nolat", "hpOverlayDisabledVehicle")) {
    html
      .find(".tidy5e-sheet.tidy5e-vehicle .profile")
