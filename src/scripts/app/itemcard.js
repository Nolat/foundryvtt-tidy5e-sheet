...
// show/hide grid layout item info card on mouse enter/leave
  ...
  let itemCardsForAllItems = game.settings.get("foundryvtt-tidy5e-sheet-nolat", "itemCardsForAllItems");
  ...
  let timer;
  let itemCardDelay = game.settings.get("foundryvtt-tidy5e-sheet-nolat", 'itemCardsDelay');
  if(!itemCardDelay || itemCardDelay == 0) itemCardDelay = false;

  let mouseOverItem = false;
  let itemCardIsFixed = false;
  let itemCardFixKey =  game.settings.get("foundryvtt-tidy5e-sheet-nolat", 'itemCardsFixKey') || "x";

  let itemCardsAreFloating = game.settings.get("foundryvtt-tidy5e-sheet-nolat", 'itemCardsAreFloating');
