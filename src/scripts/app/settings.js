...
export class Tidy5eUserSettings extends FormApplication {
  static init() {
    game.settings.registerMenu("foundryvtt-tidy5e-sheet-nolat", "userMenu", {
      name: "",
  ...
  getSettingsData() {
    // console.log(game.settings.get("foundryvtt-tidy5e-sheet-nolat"))
    const settings = [
      "ammoEquippedOnly",
      "activeEffectsMarker",
      "classListDisabled",
    ...
    settings.forEach((setting) => {
      data[setting] = { value: game.settings.get("foundryvtt-tidy5e-sheet-nolat", setting) };
  ...
  activateListeners(html) {
    ...
    html.find("input#exhaustionEffectIcon").on("change", function (e) {
      ...
      if (e.target.value == "" || e.target.value == null) {
        e.target.value =
          "modules/foundryvtt-tidy5e-sheet-nolat/images/exhaustion.svg";
      }
    });
  }
  ...
  _updateObject(ev, formData) {
    ...
    for (let key in data) {
      ...
      let oldSetting = game.settings.get("foundryvtt-tidy5e-sheet-nolat", key);
      let newSetting = data[key];
      if (oldSetting == newSetting) continue;
      // console.log(`${key} changed to "${data[key]}"`);
      game.settings.set("foundryvtt-tidy5e-sheet-nolat", key, data[key]);
...
Hooks.on("renderTidy5eUserSettings", () => {
  if (!game.user.isGM) {
    document
      .querySelectorAll(".tidy5e.settings .gm-only")
      .forEach(function (el) {
        el.remove();
      });
  }
